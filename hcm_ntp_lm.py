...
from src.model.hcm_ntp_lm import HCMNextTokenLanguageModel, token_windows
    ...
    def fit(
        ...
        for row in windows:
            ...
            for order in range(context_length + 1):
                key = tuple(int(value) for value in row[context_length - order : context_length].tolist())
    ...
    def next_token_probs(self, context_ids: torch.Tensor) -> torch.Tensor:
        ...
        for row in context_ids.tolist():
            ...
            for order in range(min(len(row), len(self.counts_by_order) - 1), -1, -1):
                key = tuple(int(value) for value in row[-order:]) if order > 0 else ()
...
class LogLinearHybridLM:
    ...
    def next_token_probs(self, context_ids: torch.Tensor) -> torch.Tensor:
        base_probs = self.base_model.next_token_probs(context_ids).clamp_min(1e-12)
        hcm_probs = self.hcm_model.next_token_probs(context_ids).clamp_min(1e-12)
        log_probs = (1.0 - self.hcm_weight) * base_probs.log() + self.hcm_weight * hcm_probs.log()
...
def generate_from_distribution_model(
    ...
    for _ in range(max_new_tokens):
        ...
        probs = model.next_token_probs(torch.tensor(context, dtype=torch.long)).squeeze(0)
        if temperature != 1.0:
        ...
        ids.append(int(torch.multinomial(probs, num_samples=1, generator=generator).item()))
    return ids


def main() -> None:
    parser = argparse.ArgumentParser(description="Direct faithful HCM/HCR next-token language model.")
    ...
    parser.add_argument("--hf-max-rows", type=int, default=None)
    ...
    parser.add_argument("--sample-model", choices=["hcm", "ngram", "hybrid"], default="hybrid")
    ...
    hcm_metrics = hcm.evaluate(val_data.data, max_windows=args.eval_windows, batch_size=args.batch_size)
    ...
    if args.sample_model == "hcm":
        ...
        sample_backend = LogLinearHybridLM(ngram, hcm, hcm_weight=args.sample_hybrid_weight)
    ...
    result = {
        ...
        "sample_model": {
            ...
            "hybrid_weight": args.sample_hybrid_weight if args.sample_model == "hybrid" else None,
        },
        "sample": sample,
    }
